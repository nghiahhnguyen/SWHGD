...
class Neo4jQuery(object):
    ...
    def run_query(self, fp, output_fp, batch_size=batch_size):
        ...
        with self._driver.session() as session:
            ...
            for i in range(1, file_row_count + 1, batch_size):
                ...
                result = session.write_transaction(self._run_query, fp=fp, output_fp=output_fp, skip=i, batch_size=batch_size)
                print(result)

    @staticmethod
    def _run_query(tx, fp, output_fp, skip, batch_size):
        query = "CALL apoc.export.csv.query(\"CALL apoc.load.csv($fp, {skip: $skip, limit: $batch_size}) yield query_revision_id, query_snapshot_id \
...
if __name__ == "__main__":
    query = Neo4jQuery()
    fp = "file:///list_of_forks_apoc_subgraph_git_master_6M_snapshots.csv"
